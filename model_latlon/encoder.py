...
class ResConvEncoder(nn.Module):
    ...
    def forward(self, x, t0):
        ...
        B,H,W,C = x.shape
        D = self.mesh.n_levels
        C_pr = self.mesh.n_pr_vars

        if self.config.oldpr:
            x_pr = self.pressure_crap(x[..., :D*C_pr])
        else:
            xv = x[..., :D*C_pr].view(B, H, W, C_pr, D)
            xs = []
            for i in range(D // 4):
                xs.append(call_checkpointed(self.pressure_crap, xv[:,:,:,:,4*i:4*(i+1)].reshape(B, H, W, C_pr*4), checkpoint_type=self.config.checkpoint_type)) 
